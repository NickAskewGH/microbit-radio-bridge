"""Decode the compact radio payload emitted by MakeCode micro:bit radio."""
...
def decode_packet(raw: bytes, *, expected_group: int | None = None) -> MakeCodePacket:
    ...
    MakeCode's radio packet starts with the three-byte DAL header
    ``version, group, protocol``. The nRF radio hardware commonly delivers a
    fixed-width 32-byte payload, so trailing zeroes are intentionally accepted.
    """
