...
static uint8_t active_makecode_group;
...
static int cmd_makecode_rx(const struct shell *shell, size_t argc, char **argv)
{
	uint32_t group;
	uint32_t channel = 7;

	if (argc < 2 || argc > 3) {
		shell_error(shell, "Usage: makecode_rx <group 0-255> [band 0-100]");
		return -EINVAL;
	}

	group = strtoul(argv[1], NULL, 10);
	if (group > 255) {
		shell_error(shell, "Group must be between 0 and 255");
		return -EINVAL;
	}
	...
	if (channel > 100) {
		shell_error(shell, "Band must be between 0 and 100");
		return -EINVAL;
	}

	if (test_in_progress && test_config.type == RX &&
	    test_config.params.rx.pattern == TRANSMIT_PATTERN_MAKECODE &&
	    test_config.params.rx.channel == channel && group == active_makecode_group) {
		shell_print(shell, "MakeCode RX already running: group %u, band %u",
			    group, channel);
		return 0;
	}
	...
	memset(&test_config, 0, sizeof(test_config));
	test_config.type = RX;
	test_config.mode = NRF_RADIO_MODE_NRF_1MBIT;
	test_config.params.rx.channel = channel;
	test_config.params.rx.pattern = TRANSMIT_PATTERN_MAKECODE;
	radio_makecode_group_set(group);
	active_makecode_group = group;
	radio_test_start(&test_config);
	test_in_progress = true;

	shell_print(shell, "MakeCode RX started: group %u, band %u", group, channel);
	return 0;
}
