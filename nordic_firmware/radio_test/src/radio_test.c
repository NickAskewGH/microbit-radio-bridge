...
static uint8_t makecode_group;

struct makecode_packet {
	uint8_t length;
	uint8_t payload[MAKECODE_MAX_PACKET_SIZE];
};

K_MSGQ_DEFINE(makecode_packet_queue, sizeof(struct makecode_packet), 8, 4);

static void makecode_packet_output(struct k_work *work)
{
	struct makecode_packet packet;

	ARG_UNUSED(work);

	while (k_msgq_get(&makecode_packet_queue, &packet, K_NO_WAIT) == 0) {
		for (uint8_t i = 0; i < packet.length; i++) {
			printk("%02x", packet.payload[i]);
		}
		printk("\n");
	}
}

K_WORK_DEFINE(makecode_packet_work, makecode_packet_output);
...
static void radio_config(nrf_radio_mode_t mode, enum transmit_pattern pattern)
{
	...
	case TRANSMIT_PATTERN_MAKECODE:
		nrf_radio_prefix0_set(NRF_RADIO, makecode_group);
		nrf_radio_base0_set(NRF_RADIO, MAKECODE_BASE_ADDRESS);
		break;
	...
	if (pattern == TRANSMIT_PATTERN_MAKECODE) {
		packet_conf.maxlen = MAKECODE_MAX_PACKET_SIZE;
		packet_conf.big_endian = false;
		nrf_radio_crc_configure(NRF_RADIO, RADIO_CRCCNF_LEN_Two,
					NRF_RADIO_CRC_ADDR_INCLUDE, MAKECODE_CRC_POLYNOMIAL);
		nrf_radio_crcinit_set(NRF_RADIO, MAKECODE_CRC_INIT);
		nrf_radio_datawhiteiv_set(NRF_RADIO, MAKECODE_DATAWHITEIV);
	}
...
void radio_handler(const void *context)
{
	...
	if (nrf_radio_int_enable_check(NRF_RADIO, NRF_RADIO_INT_CRCOK_MASK) &&
		...
		if (config->type == RX &&
		    config->params.rx.pattern == TRANSMIT_PATTERN_MAKECODE) {
			struct makecode_packet packet = {
				.length = MIN(rx_packet[0], MAKECODE_MAX_PACKET_SIZE)
			};

			memcpy(packet.payload, &rx_packet[1], packet.length);
			if (k_msgq_put(&makecode_packet_queue, &packet, K_NO_WAIT) == 0) {
				k_work_submit(&makecode_packet_work);
			}
		}
